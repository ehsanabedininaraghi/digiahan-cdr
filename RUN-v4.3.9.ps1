...
$relativeFiles = @(
    ...
    "tools\accounting-bridge-v4.3.9.ps1",
...
try {
    ...
    $dashboardConfigPath = Join-Path $sourceRoot "appsettings.Dashboard.local.json"
        ...
        Write-Host "Dashboard password hash saved to the local-only configuration." -ForegroundColor Green


   if (-not (Test-Path -LiteralPath $dashboardConfigPath)) {
        Write-Host "Set the private management-dashboard password (minimum 8 characters)." -ForegroundColor Yellow
        $securePassword = Read-Host "Dashboard password" -AsSecureString
        $passwordPointer = [Runtime.InteropServices.Marshal]::SecureStringToBSTR($securePassword)
        try { $plainPassword = [Runtime.InteropServices.Marshal]::PtrToStringBSTR($passwordPointer) }
        ...
        Write-Host "Existing dashboard password configuration preserved." -ForegroundColor DarkCyan
    }
    ...
    $dataConfig.DataGathering | Add-Member -MemberType NoteProperty -Name AccountingBridgeScript -Value "tools/accounting-bridge-v4.3.9.ps1" -Force
