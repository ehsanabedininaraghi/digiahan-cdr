...
SELECT
    p.DidarContactCode,
    CAST(NULL AS nvarchar(200)) AS RawPhone,
    p.NormalizedPhone,
    CAST(N'DIDAR' AS nvarchar(30)) AS PhoneType,
...
"@
            ...
            foreach ($code in @([string]$row.DetailCode,[string]$row.ShortCode)) {
                if (-not [string]::IsNullOrWhiteSpace($code)) {
                    $accountingCodeToIdentity[$code.Trim().ToUpperInvariant()] = $identityId
                }
            }
...
"@
        ...
        if (Test-Path $MappingsFile) {
            ...
            foreach ($mapping in $manualRows) {
                ...
                if ($accountingCode -and $accountingCodeToIdentity.ContainsKey($accountingCode)) {
                    $identityId = [int64]$accountingCodeToIdentity[$accountingCode]
                }
                elseif ($relatedPhone -and $phoneToIdentity.ContainsKey($relatedPhone)) {
                    $identityId = [int64]$phoneToIdentity[$relatedPhone]
                }
                elseif ($didarCode -and $didarCodeToIdentity.ContainsKey($didarCode)) {
                    $identityId = [int64]$didarCodeToIdentity[$didarCode]
                }
                elseif ($phoneToIdentity.ContainsKey($phone)) {
                    $identityId = [int64]$phoneToIdentity[$phone]
                }
