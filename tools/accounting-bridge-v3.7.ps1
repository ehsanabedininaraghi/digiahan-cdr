...
function Get-Configuration {
    ...
    if ([string]::IsNullOrWhiteSpace($result.AccountingLegacy)) {
        throw "ConnectionStrings:AccountingLegacy was not found."
    }

    return $result
}

function Convert-ToAdoConnectionString {
    param(
    ...
    if ([string]::IsNullOrWhiteSpace($ConnectionString)) {
        throw "AccountingLegacy connection string is empty."
    }

    if ($ConnectionString -match '(?i)\bProvider\s*=') {
    ...
    # First try the canonical SQL Server parser. It understands aliases such as
    # DataSource, InitialCatalog, UID, PWD and Trusted_Connection.
    try {
        $sqlBuilder = New-Object System.Data.SqlClient.SqlConnectionStringBuilder
        $sqlBuilder.ConnectionString = $ConnectionString

        $server = [string]$sqlBuilder.DataSource
        $database = [string]$sqlBuilder.InitialCatalog
        $user = [string]$sqlBuilder.UserID
        $password = [string]$sqlBuilder.Password
        $integrated = [bool]$sqlBuilder.IntegratedSecurity
    }
    catch {
        # Some old connection strings contain non-SqlClient keys. Fall through
        # to a tolerant generic parser.
    }
    ...
    if ([string]::IsNullOrWhiteSpace($server) -or
        ...
        try {
            ...
            function Read-ConnectionValue {
                ...
                foreach ($key in $Keys) {
                    if ($Builder.ContainsKey($key) -and $Builder[$key]) {
            ...
            if ([string]::IsNullOrWhiteSpace($server)) {
                $server = Read-ConnectionValue $builder @(
                    "Server","Data Source","DataSource","Address","Addr","Network Address"
                )
            }

            if ([string]::IsNullOrWhiteSpace($database)) {
                $database = Read-ConnectionValue $builder @(
                    "Database","Initial Catalog","InitialCatalog"
                )
            }

            if ([string]::IsNullOrWhiteSpace($user)) {
                $user = Read-ConnectionValue $builder @(
                    "User ID","UserID","UID","User"
                )
            }

            if ([string]::IsNullOrWhiteSpace($password)) {
                $password = Read-ConnectionValue $builder @(
                    "Password","PWD"
                )
            }

            $trusted = Read-ConnectionValue $builder @(
                "Integrated Security","IntegratedSecurity","Trusted_Connection","Trusted Connection"
            )

            if ($trusted -and $trusted -match '(?i)true|yes|sspi') {
                $integrated = $true
            }
        }
        catch {
            # Final fallback below uses the known Accounting settings.
        }
    }
    ...
    if ([string]::IsNullOrWhiteSpace($server) -or
        [string]::IsNullOrWhiteSpace($database)) {
        throw "Accounting source Server or Database could not be resolved."
    }

    if ($integrated -or [string]::IsNullOrWhiteSpace($user)) {
        return "Provider=SQLOLEDB;Data Source=$server;Initial Catalog=$database;Integrated Security=SSPI;"
    }

    return "Provider=SQLOLEDB;Data Source=$server;Initial Catalog=$database;User ID=$user;Password=$password;"
}
...
$sourceAdoString = Convert-ToAdoConnectionString `
    -ConnectionString $config.AccountingLegacy `
    -FallbackServer $config.AccountingServer `
    -FallbackDatabase $config.AccountingDatabase
...
try {
    Write-Host "Accounting Bridge v3.7.0" -ForegroundColor Cyan
    ...
    Write-Host "[1/6] Legacy SQL 2000 connected through SQLOLEDB." -ForegroundColor Green
...
BEGIN
    ...
    (
        ...
        FiscalYear int NULL,
        CutoffDate nvarchar(10) NULL,
        Status nvarchar(20) NULL,
        VisitorCount int NOT NULL DEFAULT(0),
        CustomerCount int NOT NULL DEFAULT(0),
        InvoiceCount int NOT NULL DEFAULT(0),
        InvoiceItemCount int NOT NULL DEFAULT(0),
        ErrorMessage nvarchar(max) NULL
    );
END;
"@
    Invoke-SqlNonQuery $sqlConnection $ensureRunTable | Out-Null

    $startRunSql = @"
INSERT INTO dbo.AccountingSyncRuns
    (RunId,StartedAtUtc,SourceServer,SourceDatabase,FiscalYear,CutoffDate,Status,
...
"@
        ...
        Write-Bulk $sqlConnection $transaction $visitors "#AccountingVisitorsStage"
        Write-Bulk $sqlConnection $transaction $customers "#AccountingCustomersStage"

        $mergeSql = @"
MERGE dbo.AccountingVisitors AS target
USING #AccountingVisitorsStage AS source
...
USING #AccountingCustomersStage AS source
ON target.SourceDatabase=source.SourceDatabase
AND target.FiscalYear=source.FiscalYear
...
"@
        ...
        Write-Bulk $sqlConnection $transaction $invoices "dbo.AccountingInvoices"
        Write-Bulk $sqlConnection $transaction $items "dbo.AccountingInvoiceItems"
...
"@
    ...
    Write-Host "[5/6] Accounting synchronization completed successfully." -ForegroundColor Green
