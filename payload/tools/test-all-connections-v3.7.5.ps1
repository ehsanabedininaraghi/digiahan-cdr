...
$textReport = Join-Path $logRoot "MVP-v3.7.5-$stamp.txt"
$jsonReport = Join-Path $logRoot "MVP-v3.7.5-$stamp.json"
...
if ($appHealthy) {
    try {
        ...
        $statusValue = if ($accountingStatus.PSObject.Properties["lastSyncStatus"]) {
            $accountingStatus.lastSyncStatus
        } else {
            $accountingStatus.lastStatus
        }

        $invoiceValue = if ($accountingStatus.PSObject.Properties["invoiceCount"]) {
            $accountingStatus.invoiceCount
        } else {
            $accountingStatus.lastInvoices
        }

        Add-Result "Accounting status API" "PASS" ("Configured={0} Status={1} Invoices={2}" -f `
            $accountingStatus.configured,$statusValue,$invoiceValue)
    }
    catch {
    ...
    if ([string]::IsNullOrWhiteSpace($settings.VoipApiToken)) {
        ...
        try {
            ...
            $appErrorLog = Join-Path $RepositoryRoot "Logs\v3.7.5-app-error.log"
            if (Test-Path $appErrorLog) {
                $tail = (Get-Content $appErrorLog -Tail 8 -ErrorAction SilentlyContinue) -join " | "
                if (-not [string]::IsNullOrWhiteSpace($tail)) {
                    $detail = "$detail | AppLog=$tail"
                }
            }

            Add-Result "VoIP event API" "FAIL" $detail
        }
    }
}
...
$lines = @()
$lines += "DigiAhan MVP Connectivity v3.7.5"
