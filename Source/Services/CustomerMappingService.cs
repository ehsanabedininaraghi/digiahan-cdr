...
public sealed class CustomerMappingService
{
    ...
    public async Task<CustomerMappingSummary> ReconcileAsync(CancellationToken ct)
    {
        await using var connection = await OpenAsync(ct);
        await EnsureSchemaAsync(connection, ct);
        await RefreshDidarPhonesAsync(connection, ct);
    ...
    private static async Task RefreshDidarPhonesAsync(SqlConnection connection, CancellationToken ct)
    {
        const string sql = "IF OBJECT_ID(N'dbo.usp_RebuildDidarContactPhones',N'P') IS NOT NULL EXEC dbo.usp_RebuildDidarContactPhones;";
        ...
        const string sql = """
            ...
            BEGIN
                ...
                (
                    ...
                    FROM Codes WHERE TRY_CONVERT(int,CleanCode) BETWEEN 1 AND 999999
                )
