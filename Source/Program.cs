    ...
    CancellationToken ct) =>
{
    ...
    return Results.Ok(await dashboard.Calls(
        start, end, string.Join(',', seller.Extensions), search, status,
...
app.MapPost("/api/system/schedules/run-all", async (
    ...
    CancellationToken ct) =>
{
    if (!CanWriteInternalData(context, configuration) && !HasDashboardAccess(context, configuration))
        return Results.Unauthorized();
