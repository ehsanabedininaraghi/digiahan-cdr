...
async function refreshAll() {
  ...
  try {
    if (new URLSearchParams(location.search).get("source") !== "api") {
      ...
      if (batchResponse.ok) {
        await loadBatchData(null, batchResponse);
        return;
      }
    }
...
function renderCoaching(coaching) {
  ...
  if (!coaching || coaching.status === "NOT_APPLICABLE") {
    box.innerHTML = `<div class="empty-coaching">این تماس برای راهنمایی فروش قابل استفاده نیست.</div>`;
    return;
  }
  const quality = coaching.quality || {};
...
function renderCoachingOverview(coaching) {
  const section = $("coachingOverview");
  if (!coaching) { section.classList.add("hidden"); return; }
  section.classList.remove("hidden");
  $("coachReadyCount").textContent = fa(coaching.readyConversationCount);
  $("transcriptReviewCount").textContent = fa(coaching.transcriptReviewCount);
  $("nonPurchaseFindingCount").textContent = fa(coaching.nonPurchaseFindingCount);
  $("sensitiveFindingCount").textContent = fa(coaching.sensitiveReviewCount);
  ...
  $("topNonPurchaseReasons").innerHTML = renderRanked(coaching.topNonPurchaseReasons);
  $("topTopics").innerHTML = renderRanked(coaching.topTopics);
